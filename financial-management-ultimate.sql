-- =============================================
-- ADVANCED FINANCIAL MANAGEMENT SCHEMA (ULTIMATE FIX)
...
-- 2. Payment Terms (No dependencies)
...
-- 3. Exchange Rates (Depends on currencies)
...
-- 4. Chart of Accounts (Self-referencing with parent_account_id)
CREATE TABLE chart_of_accounts (
    ...
    parent_account_id UUID, -- Self-reference to chart_of_accounts
    is_active BOOLEAN DEFAULT TRUE,
...
-- 5. Tax Configuration (Depends on chart_of_accounts)
...
-- 6. Financial Transactions (Depends on currencies)
...
-- 7. Journal Entries (Depends on financial_transactions and chart_of_accounts)
...
-- 8. Banking Integration (Depends on currencies and chart_of_accounts)
...
-- 9. Bank Transactions (Depends on bank_accounts and financial_transactions)
...
-- 10. Budget Planning (Depends on currencies)
...
-- 11. Budget Line Items (Depends on budget_plans and chart_of_accounts)
...
-- Add self-reference foreign key for chart_of_accounts
ALTER TABLE chart_of_accounts 
ADD CONSTRAINT fk_chart_accounts_parent 
FOREIGN KEY (parent_account_id) REFERENCES chart_of_accounts(id);

-- Add foreign keys for tax_codes
...
-- Add foreign keys for financial_transactions
ALTER TABLE financial_transactions 
ADD CONSTRAINT fk_financial_transactions_currency 
FOREIGN KEY (currency_id) REFERENCES currencies(id);

-- Add foreign keys for journal_entries
...
-- Add foreign keys for bank_accounts
...
-- Add foreign keys for budget_plans
ALTER TABLE budget_plans 
ADD CONSTRAINT fk_budget_plans_currency 
FOREIGN KEY (currency_id) REFERENCES currencies(id);

-- Add foreign keys for budget_line_items
...
-- Add auth.users foreign keys conditionally
DO $$ 
BEGIN
    -- Check if auth.users exists before adding foreign keys
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auth' AND table_name = 'users') THEN
        -- Add foreign keys for auth.users references
        ALTER TABLE chart_of_accounts 
        ...
        RAISE NOTICE 'Auth user foreign key constraints added successfully!';
    ELSE
        RAISE NOTICE 'auth.users table not found - user foreign keys will be added when available';
    END IF;
END $$;
...
CREATE POLICY "Users can view chart of accounts" ON chart_of_accounts FOR SELECT USING (true);

DROP POLICY IF EXISTS "Users can view tax codes" ON tax_codes;
CREATE POLICY "Users can view tax codes" ON tax_codes FOR SELECT USING (true);
...
DROP TRIGGER IF EXISTS update_chart_accounts_updated_at ON chart_of_accounts;
...
-- Trigger to validate journal balance (commented out to avoid immediate validation)
...
GRANT SELECT ON tax_codes TO authenticated;
...
DO $$ BEGIN
    ...
    RAISE NOTICE 'All tables created with proper column definitions!';
    RAISE NOTICE 'Foreign keys added in correct order!';
    RAISE NOTICE 'Self-referencing parent_account_id working correctly!';
END $$;
