...
-- DESIGNED FOR LARGE CORPORATION OPERATIONS
-- =============================================
...
CREATE TABLE IF NOT EXISTS customers (
    ...
    sales_rep_id UUID REFERENCES employee_profiles(id),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS purchase_orders (
    ...
    buyer_id UUID REFERENCES employee_profiles(id),
    order_date DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS compliance_checks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    entity_type VARCHAR(50) NOT NULL, -- 'shipment', 'customer', 'commodity', etc.
...
-- Core indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_employee_profiles_user_id ON employee_profiles(user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_employee_profiles_department ON employee_profiles(department_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_notifications_recipient ON notifications(recipient_id, read, created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stock_inventory_commodity ON stock_inventory(commodity_id, warehouse_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sales_orders_customer ON sales_orders(customer_id, order_date DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_invoices_status_due ON invoices(status, due_date);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_audit_logs_table_record ON audit_logs(table_name, record_id, timestamp DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_shipments_status ON shipments(status, departure_date);

-- Composite indexes for complex queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_stock_commodity_warehouse_status ON stock_inventory(commodity_id, warehouse_id, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sales_orders_status_date ON sales_orders(status, order_date DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payments_invoice_date ON payments(invoice_id, payment_date DESC);
...
-- Daily stock valuation update
SELECT cron.schedule('daily-stock-valuation', '0 1 * * *', 
    'SELECT calculate_stock_valuation();');

-- Weekly customer aging report
SELECT cron.schedule('weekly-aging-report', '0 9 * * 1', 
    'SELECT calculate_customer_aging();');

-- Daily cleanup of old notifications (older than 90 days)
...
SELECT 
    ...
    ep.full_name as sales_rep,
    COUNT(so.id) as total_orders,
    SUM(so.total_amount) as total_sales,
    AVG(so.total_amount) as average_order_value,
    COUNT(DISTINCT so.customer_id) as unique_customers
FROM sales_orders so
JOIN employee_profiles ep ON so.sales_rep_id = ep.id
...
JOIN commodities c ON soi.commodity_id = c.id
WHERE so.status IN ('approved', 'shipped', 'delivered')
GROUP BY DATE(so.order_date), c.code, c.name
ORDER BY sale_date DESC, total_value DESC;

-- Create unique index for materialized view refresh
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_daily_sales_unique 
ON mv_daily_sales (sale_date, commodity_code);

-- Schedule daily refresh of materialized view
SELECT cron.schedule('refresh-daily-sales', '0 6 * * *',
...
BEGIN
    ...
    RAISE NOTICE '⚡ Performance: Indexes, Materialized views, Scheduled jobs';
