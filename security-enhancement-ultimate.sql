-- =============================================
-- SECURITY ENHANCEMENT SCHEMA FOR ITM TRADING (ULTIMATE FIX)
...
-- Add foreign keys only if auth.users table exists
DO $$ 
BEGIN
    -- Check if auth.users exists before adding foreign keys
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auth' AND table_name = 'users') THEN
        -- Add foreign keys for user_security_settings
        ALTER TABLE user_security_settings 
        ADD CONSTRAINT fk_user_security_user 
        FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
        
        -- Add foreign keys for user_sessions
        ALTER TABLE user_sessions 
        ADD CONSTRAINT fk_user_sessions_user 
        FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
        
        -- Add foreign keys for security_events
        ALTER TABLE security_events 
        ADD CONSTRAINT fk_security_events_user 
        FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE SET NULL;
        
        -- Add foreign keys for compliance_audit
        ALTER TABLE compliance_audit 
        ADD CONSTRAINT fk_compliance_audit_user 
        FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE SET NULL;
        
        -- Add foreign keys for risk_assessments
        ALTER TABLE risk_assessments 
        ADD CONSTRAINT fk_risk_assessments_user 
        FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;
        
        ALTER TABLE risk_assessments 
        ADD CONSTRAINT fk_risk_assessments_approver 
        FOREIGN KEY (approved_by) REFERENCES auth.users(id);
        
        RAISE NOTICE 'Foreign key constraints added successfully!';
    ELSE
        RAISE NOTICE 'auth.users table not found - foreign keys will be added when available';
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Security admins can view all events" ON security_events;
...
DROP POLICY IF EXISTS "Compliance officers can view all audit logs" ON compliance_audit;
...
DROP POLICY IF EXISTS "System can insert audit logs" ON compliance_audit;
CREATE POLICY "System can insert audit logs" ON compliance_audit
    FOR INSERT WITH CHECK (true);
...
DO $$ BEGIN
    ...
    RAISE NOTICE 'Foreign keys added conditionally based on auth.users existence!';
END $$;
